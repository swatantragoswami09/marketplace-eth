...
const createFormState = ({ price, email, confirmationEmail }, hasAgreedTOS) => {
  if (!price || Number(price) <= 0) {
    ...
    return _createFormState(true, "Emails are not mathcing.");
  } else if (!hasAgreedTOS) {
    return _createFormState(
      true,
      "Your need to agree with Terms of Service in order to submit the form"
    );
  }
        ...
        <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
          <div className="sm:flex sm:items-start">
            <div className="mt-3 sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <label className="flex items-center mr-2">
                      <input
                        ...
                        onChange={({ target: { checked } }) => {
                          setOrder({
                            ...order,
                            price: checked ? order.price : eth.preItem,
                          });
              ...
              {formState.message && (
                <div className="py-4 my-3 text-yellow-700 bg-yellow-200 rounded-lg text-sm">
                  {formState.message}
                </div>
              )}
            </div>
